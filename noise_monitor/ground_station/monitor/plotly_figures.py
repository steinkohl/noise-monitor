...
def create_3d_figure(sweep_df: pd.DataFrame) -> go.Figure:
    ...
    if bandwidth.size > 1:
        raise Exception(f'Expected only one resolution bandwidth within the same sweep file, but got {bandwidth}')
    bandwidth = bandwidth[0]
    fig.add_trace(
        go.Scatter3d(x=sweep_df["measurement_azimuth"],
    ...
    psd_bins = sum([1 if "psd" in n else 0 for n in sweep_df.columns]) - 4  # -(min, mean, max, bandwidth)
...
def create_contour_figure(sweep_df: pd.DataFrame, controller: GroundStationController) -> (go.Figure, Position):
    fig = go.Figure()
    bandwidth = sweep_df["psd_bandwidth"].unique()
    if bandwidth.size > 1:
        raise Exception(f'Expected only one resolution bandwidth within the same sweep file, but got {bandwidth}')
    ...
    max_sigs = sweep_df[sweep_df["psd_min"] > sweep_df["psd_min"].quantile(.95)]
    ...
    fig.add_shape(
        type="line", x0=x_max, y0=y.min(), x1=x_max, y1=y.max(),
        line=dict(
            color="black",
            width=4,
            dash="dot",
    ...
    fig.add_shape(
        type="line", x0=x.min(), y0=y_max, x1=x.max(), y1=y_max,
        line=dict(
            color="black",
            width=4,
            dash="dot",
    ...
    fig.add_shape(type="circle",
                  xref="x", yref="y",
                  x0=x_max - angle_az / 2, y0=y_max - angle_el / 2,
                  x1=x_max + angle_az / 2, y1=y_max + angle_el / 2,
    ...
    if not sun_df.empty:
        fig.add_trace(
            go.Scatter(
                y=sun_df['el'], x=sun_df['az'],
                mode='lines', line_shape='spline',
                line_color='red', name='Sun Path',
            )
        )
    fig.update_layout(
        {"xaxis": {"title": "Azimuth [°]"},
